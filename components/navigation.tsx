...
export const MobileNav = (props: {open:boolean}) => {
  ...
  const navLinks = links.map(({path, text}, index) => {
    return <li className={`whitespace-nowrap font-medium hover:text-green-100 ${checkPath === path.toLowerCase() ? "text-green-100" : "text-blue-400 dark:text-white"}`} key={index}>
